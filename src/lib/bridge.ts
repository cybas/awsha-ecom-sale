...
export function bridgeUrl(sku: string, qty = 1, go: "cart" | "checkout" = "cart", extras?: Record<string,string>) {
  ...
  if (extras) for (const [k, v] of Object.entries(extras)) u.searchParams.set(k, v);
  return u.toString();
}
